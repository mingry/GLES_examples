...
void InitOpenGL()
{
	s_program_id = CreateFromFiles("../Shaders/v_shader_gooch.glsl", "../Shaders/f_shader_pbr.glsl");
...
void Display()
{
	...
	glUniform1f(glGetUniformLocation(s_program_id, "ao"), 1.0f);

	// 바닥 격자
	glUniform1f(glGetUniformLocation(s_program_id, "metallic"), 0.8f);
	glUniform1f(glGetUniformLocation(s_program_id, "roughness"), 0.5f);
	...
	// Moving Car
	{
		glUniform1f(glGetUniformLocation(s_program_id, "metallic"), 1.f);
		glUniform1f(glGetUniformLocation(s_program_id, "roughness"), 0.2f);
	...
	for (int i = 0; i < 4; i++)
	{
		lightPositions[i] = glm::vec3( view_matrix * glm::vec4(lightPositions[i], 1.f) );
	}

	glUniform3fv(glGetUniformLocation(s_program_id, "lightPositions[0]"), 1, &lightPositions[0][0]);
	glUniform3fv(glGetUniformLocation(s_program_id, "lightPositions[1]"), 1, &lightPositions[1][0]);
	glUniform3fv(glGetUniformLocation(s_program_id, "lightPositions[2]"), 1, &lightPositions[2][0]);
	glUniform3fv(glGetUniformLocation(s_program_id, "lightPositions[3]"), 1, &lightPositions[3][0]);
	glUniform3fv(glGetUniformLocation(s_program_id, "lightColors[0]"), 1, &lightColors[0][0]);
	glUniform3fv(glGetUniformLocation(s_program_id, "lightColors[1]"), 1, &lightColors[1][0]);
	glUniform3fv(glGetUniformLocation(s_program_id, "lightColors[2]"), 1, &lightColors[2][0]);
	glUniform3fv(glGetUniformLocation(s_program_id, "lightColors[3]"), 1, &lightColors[3][0]);
	...
	for (int i = 0; i < 4; i++)
	{
		glUniform1f(glGetUniformLocation(s_program_id, "metallic"), (float)i / 4.f);
		for (int j = 0; j < 4; j++)
		{
			glUniform1f(glGetUniformLocation(s_program_id, "roughness"), glm::clamp((float)j / 4.f, 0.05f, 1.f));
			...
			glVertexAttrib4f(2, 1.f, 0.f, 0.f, 1.f);
			DrawSphere();
